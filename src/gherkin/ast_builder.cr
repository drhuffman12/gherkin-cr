...
module Gherkin
  class AstBuilder
    ...
    def initialize
      ...
      @comments = [] of Hash(Symbol, Hash(Symbol, Int32) | String | Symbol | Nil) # Gherkin::Token
    end
    ...
    def start_rule(rule_type)
      raise "Nil Stack" if @stack.nil?
      @stack.push AstNode.new(rule_type) unless @stack.nil?
    end
    ...
    def get_location(token, column=nil) # : LocationType
