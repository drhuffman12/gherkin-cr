module Gherkin
  module Pickles
    class Compiler
      ...
      private def compile_scenario(feature_tags, background_steps, scenario, language, pickles)
      ...
      private def compile_scenario_outline(feature_tags, background_steps, scenario_outline, language, pickles)
      ...
      private def create_pickle_arguments(argument, variable_cells, value_cells)
        result = []
      ...
      private def interpolate(name, variable_cells, value_cells)
      ...
      private def pickle_steps(scenario_definition)
        scenario_definition[:steps].map do |step|
          pickle_step(step)
        end
      end

      private def pickle_step(step)
        {
          :text => step[:text],
      ...
      private def pickle_step_location(step)
        {
          :line => step[:location][:line],
          :column => step[:location][:column] + (step[:keyword] ? step[:keyword].size : 0)
        }
      end

      private def pickle_location(location)
        {
          :line => location[:line],
